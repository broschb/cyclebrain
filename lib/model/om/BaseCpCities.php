...
/**
 * Base class that represents a row from the 'cp_cities' table.
 *
 * 
 *
 ...
 */
abstract class BaseCpCities extends BaseObject  implements Persistent {


  const PEER = 'CpCitiesPeer';
	...
	/**
	 * Initializes internal state of BaseCpCities object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     CpCities The current object (for fluent API support)
	 */
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = CpCitiesPeer::ID;
		}
	...
	public function setStateId($v)
	{
		...
		if ($this->state_id !== $v) {
			$this->state_id = $v;
			$this->modifiedColumns[] = CpCitiesPeer::STATE_ID;
		}
	...
	public function setName($v)
	{
		...
		if ($this->name !== $v) {
			$this->name = $v;
			$this->modifiedColumns[] = CpCitiesPeer::NAME;
		}
	...
	public function setLatitude($v)
	{
		...
		if ($this->latitude !== $v) {
			$this->latitude = $v;
			$this->modifiedColumns[] = CpCitiesPeer::LATITUDE;
		}
	...
	public function setLongitude($v)
	{
		...
		if ($this->longitude !== $v) {
			$this->longitude = $v;
			$this->modifiedColumns[] = CpCitiesPeer::LONGITUDE;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 5; // 5 = CpCitiesPeer::NUM_COLUMNS - CpCitiesPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating CpCities object", $e);
		}
	}
	...
	public function ensureConsistency()
	{
		...
		if ($this->aCpStates !== null && $this->state_id !== $this->aCpStates->getId()) {
			$this->aCpStates = null;
		}
	} // ensureConsistency
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = CpCitiesPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
    ...
    foreach (sfMixer::getCallables('BaseCpCities:delete:pre') as $callable)
    {
      ...
      if ($ret)
      {
		...
		try {
			CpCitiesPeer::doDelete($this, $con);
    ...
    foreach (sfMixer::getCallables('BaseCpCities:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseCpCities:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseCpCities:save:post') as $callable)
    {
      call_user_func($callable, $this, $con, $affectedRows);
    }

			CpCitiesPeer::addInstanceToPool($this);
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollBack();
			throw $e;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = CpCitiesPeer::doInsert($this, $con);
					...
					$affectedRows += CpCitiesPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = CpCitiesPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = CpCitiesPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = CpCitiesPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(CpCitiesPeer::ID)) $criteria->add(CpCitiesPeer::ID, $this->id);
		if ($this->isColumnModified(CpCitiesPeer::STATE_ID)) $criteria->add(CpCitiesPeer::STATE_ID, $this->state_id);
		if ($this->isColumnModified(CpCitiesPeer::NAME)) $criteria->add(CpCitiesPeer::NAME, $this->name);
		if ($this->isColumnModified(CpCitiesPeer::LATITUDE)) $criteria->add(CpCitiesPeer::LATITUDE, $this->latitude);
		if ($this->isColumnModified(CpCitiesPeer::LONGITUDE)) $criteria->add(CpCitiesPeer::LONGITUDE, $this->longitude);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(CpCitiesPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of CpCities (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     CpCities Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 * Otherwise if this CpCities has previously been saved, it will retrieve
	 * related UserProfiles from storage. If this CpCities is new, it will return
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseCpCities:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseCpCities::%s', $method));
    }
